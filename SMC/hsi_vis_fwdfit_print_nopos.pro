PRO HSI_VIS_FWDFIT_PRINT_nopos, srcstr, COMPACT=compact, _EXTRA=_extra
  ;
  ...
  ; 12-Aug-19,Kim Added one more significant figure to printout of eccentricity
  ;
  IF KEYWORD_SET(compact) EQ 0 THEN BEGIN
    PRINT
    PRINT, 'COMPONENT  PROFILE      FLUX       <FWHM>  Eccentricity  PosnAngle   Loop_FWHM AlbedoRatio   SrcHeight'
    PRINT, '                      ph/cm2/s     arcsec                deg EofN     degrees                    Mm'
    PRINT
  ENDIF
  nsrc = N_ELEMENTS(srcstr)
  FOR n = 0, nsrc-1 DO BEGIN
    temp        = [ srcstr[n].srcflux,  $
      srcstr[n].srcfwhm,  srcstr[n].eccen, srcstr[n].srcpa, srcstr[n].loop_angle, $
      srcstr[n].albedo_ratio, srcstr[n].srcheight]
    PRINT, n+1, srcstr[n].srctype, temp, FORMAT="(I5, A13, F12.2, 1F10.2, F10.3, 2F12.1, F12.3, F12.1)"
  ENDFOR
