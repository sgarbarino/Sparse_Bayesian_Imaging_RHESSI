FUNCTION stix_display_pixels_trans,sci_file,bkg_file,tr_flare,er_flare,stop=stop,xy_flare=xy_flare,no_trans=no_trans,rdata=rdata, silent=silent

default, silent, 0

;extracts pixel data and displays top and bottom pixel sets 

;in addtion to the earlier routing stix_display_pixels, this version also corrects for grid transmission
...
;  output is summed counts/s/keV for all pixels
;  rdata = are counts/s/keV for all times, pixels, and energies
;  
;read data
...
;background is taken after the flare
  ...
  loadct,5
  window,2,xsize=720,ysize=500
  clearplot
  if n_elements(elist) ne 1 then begin
    utplot,time_spec,total(total(total(rspec_all(*,g01_10,*,elist),4),3),2),psym=10,ytitle='STIX count rate [s!U-1!N]',title=range_title, charsize=charsize
    ...
    utplot,time_spec,total(total(rspec_all(*,g01_10,*,elist),3),2),psym=10,ytitle='STIX count rate [s!U-1!N]',title=range_title, charsize=charsize
    outplot,time_spec,time_spec*0.+total(rspec_all_bkg(0,g01_10,*,elist)),color=166
    outplot,time_spec(tlist),total(total(rspec_all(tlist,g01_10,*,elist),3),2),psym=10,color=122
  endelse
  ;makegif
  ;spawn,'mv plot.gif pixel_time.gif'
  endif
  ;subtract background
 ...
 ;make output of count spectra and bkg of all times, pixels, and energies in cts/s/keV
 ;apply grid trans to background subtracted count spectra (background is from calibration source)
 rspec=rspec_all_k
 drspec=drspec_all_k
 bspec=rspec_all_bkg_k
 rspec_sub=rspec
 for i=0,n_elements(time_spec)-1 do rspec_sub(i,*,*,*)=rspec(i,*,*,*)-bspec
 rspec_sub_trans=rspec_sub
 for i=0,31 do rspec_sub_trans(*,i,*,*)=rspec_sub_trans(*,i,*,*)/this_gtrans(i)
 rdata={rspec_sub: rspec_sub, rspec_sub_trans: rspec_sub_trans, rspec: rspec, drspec: drspec, bspec: bspec, time: time_spec, e0: e0, e1: e1, e: ee, de: ddee}
...
;  ;plot with orientations
;  if ~silent then begin
;  window,4,xsize=480,ysize=800,xpos=0,ypos=40
;  this_r=(1./res32)^2
;  this_range=[0,5d-3]
;  this_r=res32
;  this_range=[0,200]
;  xx32=sin(o32/180.*!pi)*this_r
;  yy32=cos(o32/180.*!pi)*this_r
;  
;  plot,xx32(g03_10),yy32(g03_10),psym=1,xrange=this_range,yrange=[-this_range(1),this_range(1)]
;  for i=10,170,20 do oplot,sin(i/180.*!pi)*res10,cos(i/180.*!pi)*res10
;  for i=0,9 do oplot,sin(findgen(181)/180.*!pi)*res10(i),cos(findgen(181)/180.*!pi)*res10(i)
;  ;oplot,xx32(g03_10),yy32(g03_10),psym=1,symsize=1.8,color=122
;  ;color each point according to value in amplitude
;  this_aa=(amp_top+amp_bot)/2.
;  this_a=alog10((amp_top+amp_bot)/2.)
;  this_a_range=max(this_a)-min(this_a)
;  this_c=(this_a-min(this_a))*(235./this_a_range)+10
;  loadct,39
;  ;for i=0,23 do plots,xx32(g03_10(i)),yy32(g03_10(i)),psym=1,symsize=2.8,color=this_c(g03_10(i)),thick=3
;  for i=0,23 do oplot,[xx32(g03_10(i)),xx32(g03_10(i))],[yy32(g03_10(i)),yy32(g03_10(i))],psym=1,symsize=2.8,color=this_c(g03_10(i)),thick=3
;  if keyword_set(stop) then stop
; ;interpolate in angle for same resolution
;    oplot_int_res,g10,o32,res32,this_a,this_aa,this_a_range  
;    oplot_int_res,g09,o32,res32,this_a,this_aa,this_a_range
;    oplot_int_res,g08,o32,res32,this_a,this_aa,this_a_range
;    oplot_int_res,g07,o32,res32,this_a,this_aa,this_a_range
;    oplot_int_res,g06,o32,res32,this_a,this_aa,this_a_range
;    oplot_int_res,g05,o32,res32,this_a,this_aa,this_a_range
;    oplot_int_res,g04,o32,res32,this_a,this_aa,this_a_range
;    oplot_int_res,g03,o32,res32,this_a,this_aa,this_a_range
;  endif
  ...
  ;save,p6_cts,p6_cts_e,p6_rate,p6_rate_e,tr_flare,er_flare,g10,g09,g08,g07,g06,g05,g04,g03,g02,g01,filename='/Users/admin/Documents/PhD/Papers/STIX Imaging group/STIX demonstration/Data/pixel_'+gh_filename
  return,out
