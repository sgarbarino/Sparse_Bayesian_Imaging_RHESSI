...
function vis_bayes_compute_peaks, Nsamp, H
...
function vis_bayes_data_stack, param,  num_sources,  sample, weights, H, types, weight_type=weight_type, pc_pe_pl=pc_pe_pl

  ; Data stacking
  mean_param = make_array(param.Nsamp,param.Nsamp,param.N_param)
  weight_tmp = make_array(param.Nsamp,param.Nsamp)
  weight_type = make_array(param.Nsamp,param.Nsamp, 3) ; c'e' un 3 perche' ho tre strutture possibili? deve diventare 4?
  pc_pe_pl = make_array(param.Nsamp, param.Nsamp, 3) 

; ricordo l'ordinamento dei parametri:
; 0 param.sigma_pos
; 1 param.sigma_pos, 
; 2 param.sigma_pa, 
; 3 param.sigma_ecc, 
  ...
  for p = 0, param.N_particles-1 do begin
    for k = 0, num_sources[p]-1 do begin
      ...
      if (r ge 0) and (r lt param.Nsamp) and (s ge 0) and (s lt param.Nsamp) then begin ; if source is inside the fov
        ...
        mean_param[s,r,par_tmp] = mean_param[s,r,par_tmp] + sample[k,par_tmp,p]*weights[p] ; for positions, fwhm and flux (1 and 2) it is just the mean

        ; Compute the mean value (according to the shape) and the number of circles, ellipses, and loops
        
          if (sample[k,3,p] lt param.ecc_toll_min) then begin ; se l'eccentricita' e' bassa abbastanza allora e' un cerchio
            weight_type[s,r,0] =  weight_type[s,r,0] + weights[p]
            pc_pe_pl[s,r,0] = pc_pe_pl[s,r,0]+1
          endif else if ((sample[k,3,p] ge param.ecc_toll_min) and (sample[k,3,p] lt param.ecc_toll_max)) then begin ;; se invece e' nel range ...
            if (abs(sample[k,6,p]) lt param.loop_toll_max) then begin ; ... e se curvatura ||<max, e' ellisse
              weight_type[s,r,1] =  weight_type[s,r, 1] + weights[p]
              pc_pe_pl[s,r,1] = pc_pe_pl[s,r,1]+1
            endif else begin                                          ; altrimenti e' loop
              weight_type[s,r,2] =  weight_type[s,r,2] + weights[p]
              pc_pe_pl[s,r,2] = pc_pe_pl[s,r,2]+1
            endelse
          endif else if (sample[k,3,p] ge param.ecc_toll_max) then begin ; se invece l'eccentricita' e' oltre il max
            if (abs(sample[k,6,p]) ge param.loop_toll_min) then begin    ; e la curvatura ||>min, e' loop
              weight_type[s,r,2] =  weight_type[s,r,2] + weights[p]
              pc_pe_pl[s,r,2] = pc_pe_pl[s,r,2]+1
            endif else begin                                              ; se eccentricita' e' oltre il max, e' ellisse
              weight_type[s,r,1] =  weight_type[s,r,1] + weights[p]
              pc_pe_pl[s,r,1] = pc_pe_pl[s,r,1]+1
            endelse
          endif
...
function vis_bayes_reconstr_vis, param, distr_num_src,  H, jdum, mean_param,  shape,  visxyobs, visxyrec=visxyrec, sample, N_src_expected=N_src_expected, weight_type, pc_pe_pl, prop_cel_100=prop_cel_100, time_title, phase=phase, cerberus=cerberus
...
;print,cerberus
  ...
  if (N_src_expected ge 1) then begin
    ...
    peaks=vis_bayes_compute_peaks(param.Nsamp, H) ; find the local maximum of the distribution

    ; Initialization
    visxyrec_tmp = dblarr(N_elements(jdum))
    if (phase eq 0) then visxyrec = visxyrec_tmp[0:N_ELEMENTS(visxyrec_tmp)/2-1]
    if (phase eq 1) then visxyrec = visxyrec_tmp
    ...
    for sorg = 0, N_src_expected-1 do begin
      ...
      ; Determine the index where there is the maximum of the distribution (for position)
      j = peaks[0,sorg]
      i = peaks[1,sorg]

      ; According to the shape, determine the eccentricity, pa, and loop angle 
      if (weight_type[j,i,1] ge weight_type[j,i,0]) and (weight_type[j,i,1] ge weight_type[j,i,2]) then begin ; elipse
        pa = mean_param[j,i,2]
        ecc = mean_param[j,i,3]
      endif else if (weight_type[j,i,2] ge weight_type[j,i,0]) and (weight_type[j,i,2] ge weight_type[j,i,1]) then begin ;loop
        pa = mean_param[j,i,2]
        ecc = mean_param[j,i,3]
        la = mean_param[j,i,6]
      endif
      ...
      visxyrec_tmp  += vis_fwdfit_func(jdum, srcparm)
      
      if (phase eq 0) then begin
        visxyrec_complex = complex(visxyrec_tmp[0:N_ELEMENTS(visxyrec_tmp)/2-1],visxyrec_tmp[N_ELEMENTS(visxyrec_tmp)/2:*])
        visxyrec = abs(visxyrec_complex)
      endif
      if (phase eq 1) then visxyrec = visxyrec_tmp
...
;
;    ;VIS_SOURCE2MAP, srcstrin, mapcenter, data,pixel=param.pixel_size, mapsize = param.fov/param.pixel_size ; _extra={pixel:pixel, mapsize : fov/pixel}
;    ;map_asmc    = MAKE_MAP(data, xc=mapcenter[0], yc=mapcenter[1], dx=param.pixel_size, dy=param.pixel_size, time=anytim(time_title,/ecs))
;
;     map_asmc = asmc_bayes_showmap_stix(param.fov,param.pixel_size, mapcenter, smcsources, phase=phase, cerberus=cerberus)
;
;     loadct, 5
;     window, 1
...
function vis_bayes_sort_part, param, n_src, smcsources, sample, Nsources, weights=weights, wei_s=wei_s
  COMMON uvdata, u,v, pa, mapcenter
...
function vis_bayes_func, param, Nsources,  weights, fl,  sample, H, types, jdum,  shape,  visxyobs, visxyrec=visxyrec, N_src_expected=N_src_expected, smcsources, wei_s=wei_s, par_s=par_s, time_title, phase=phase, cerberus=cerberus
...
;print, cerberus
  if fl eq 1 then begin ; when the algorithm converges, compute the recovered sources and the parameters of each particle.
    ...
    mean_param = vis_bayes_data_stack(param, Nsources,  sample,  weights, H, types, weight_type=weight_type, pc_pe_pl=pc_pe_pl)
    smcsources = vis_bayes_reconstr_vis(param, distr_num_src, H, jdum, mean_param,  shape,  visxyobs, visxyrec=visxyrec,  sample, N_src_expected=N_src_expected, weight_type, pc_pe_pl,  prop_cel_100=prop_cel_100, time_title, phase=phase, cerberus=cerberus)
    par_s = vis_bayes_sort_part(param, N_src_expected, smcsources, sample,  Nsources, weights=weights, wei_s=wei_s)
